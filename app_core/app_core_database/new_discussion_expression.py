...
def new_discussion_expression_node(
	...
	transaction.append("CREATE (a:DiscussionExpressionGraph{discussion_expression_id : " + discussion_expression_id + "})")
	return transaction
...
def new_discussion_expression_relation(
	...
	transaction.append("MATCH (e:ExpressionGraph{expression_id: " + discussion_parent_id + " }), (p:Person{person_id: '" + discussion_expression_owner_id + "' }) , (d:DiscussionExpressionGraph{discussion_expression_id: " + discussion_expression_id + " })  CREATE (p)-[:DISCUSSED]->(d)-[:ON_EXPRESSION]->(e)")
	return transaction
